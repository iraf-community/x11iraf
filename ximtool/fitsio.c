...
static char *ftgbyte (FITS *fs, uchar *buffer, int nelem, float *z1, float *z2);
 ...
 */
char *
loadFITS (fname, pix, nx, ny, r,g,b, ncolors, z1, z2)
...

{
	...
	error = ftgbyte (&fs, image, np, z1, z2);
	ftclose (&fs);
	if (error) {
	    free (image);
	    return error;
 ...
 */
static char	*rdcard(card, name, dtype, kvalue, rvalue)
char	*card, *name;
enum datatype dtype;   /* type of value */
long int	*kvalue;
float 	*rvalue;
 ...
 */
static char	*ftgbyte(fs, cbuff, nelem, z1, z2)
FITS *fs;
uchar	*cbuff;
int	nelem;
float 	*z1, *z2;
