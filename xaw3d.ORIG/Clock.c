    ...
    Region region;		/* unused */
{
    ...
    /* daisychain to the 3d class expose method to draw shadows */
    (*cwclass->threeD_class.shadowdraw) (gw, event, region, TRUE);
  ...
  XtIntervalId *id;
{
  ...
  if (id || !w->clock.interval_id)
    w->clock.interval_id =
      XtAppAddTimeOut( XtWidgetToApplicationContext( (Widget) w),
			(unsigned long) w->clock.update*1000, clock_tic, (XtPointer)w );
  (void) time(&time_value);
  tm = *localtime(&time_value);
  /*
  ...
  if( w->clock.analog == FALSE ) {
    ...
    for (i = 0; ((i < len) && (i < prev_len) && 
      ...
      (void) strcpy (w->clock.prev_time_string+i, time_ptr+i);
    ...
    if (clear_from < (int)w->core.width)
      XFillRectangle (dpy, win, w->clock.EraseGC,
			clear_from, 
			(int) w->threeD.shadow_width, 
			w->core.width - clear_from - w->threeD.shadow_width, 
			w->core.height - (2 * w->threeD.shadow_width));
  } else {
    /*
    ...
    if (w->clock.numseg == 0 || tm.tm_min != w->clock.otm.tm_min ||
       ...
       * line algorithm.
       */
      DrawHand(w, w->clock.minute_hand_length, 
               w->clock.hand_width, tm.tm_min * 12);
      if(w->clock.Hdpixel != w->core.background_pixel)
        XFillPolygon( dpy, win, w->clock.HandGC, w->clock.segbuff, 
		      VERTICES_IN_HANDS, Convex, CoordModeOrigin);
      XDrawLines( dpy, win, w->clock.HighGC, w->clock.segbuff, 
      ...
      if(w->clock.Hdpixel != w->core.background_pixel)
        XFillPolygon(dpy, win, w->clock.HandGC, w->clock.hour, 
		     VERTICES_IN_HANDS, Convex, CoordModeOrigin);
      XDrawLines( dpy, win, w->clock.HighGC, w->clock.hour, 
		  VERTICES_IN_HANDS, CoordModeOrigin );
      w->clock.sec = w->clock.segbuffptr;
    }
    if (w->clock.show_second_hand == TRUE) {
      w->clock.segbuffptr = w->clock.sec;
      DrawSecond(w, w->clock.second_hand_length - 2, 
		 w->clock.second_hand_width,
		 w->clock.minute_hand_length + 2,
		 tm.tm_sec * 12);
      if(w->clock.Hdpixel != w->core.background_pixel)
	XFillPolygon( dpy, win, w->clock.HandGC, w->clock.sec,
		      VERTICES_IN_HANDS -2, Convex, CoordModeOrigin);
      XDrawLines( dpy, win, w->clock.HighGC, w->clock.sec,
